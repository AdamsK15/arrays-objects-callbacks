...
function looper(rollCall) {
  let fam = []
  for (let i = 0; i < rollCall.length; i++) {
    fam.push(alert(rollCall[i]))
  }
  return fam;
}
let famList = looper(family)
...
//Code Here
function reversedLooper(NotLetters) {
  let srettel = [];
  for (let i = NotLetters.length - 1; i >= 0; i--) {
    srettel.push(alert(NotLetters[i]))
  }
  return srettel;
}
let swap = reversedLooper(letters)
